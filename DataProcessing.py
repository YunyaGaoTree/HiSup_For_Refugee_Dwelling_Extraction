...
def select_bands(image_data):
    
    blue = image_data[...,:1]
    green = image_data[...,1:2]
    red = image_data[...,2:3]
    image_new = np.concatenate((red, green, blue), axis=-1)
    
    return image_new

def convert_to_coco(path_dataset, data_type):
    # data_type: train or test
    # install("imagecodecs")

    input_image_path = path_dataset + '/raw/'+data_type+'/images/'
    input_gt_path = path_dataset + '/raw/'+data_type+'/gt/'
    save_path = path_dataset + '/'+data_type+'/'

    # cities = [city]
    val_set = [] #[str(i) for i in range(1, 6)]

    output_im_train = os.path.join(save_path, 'images')
    output_gt_train = os.path.join(save_path, 'masks')
    ...
    for g_id, label in enumerate(tqdm(input_label)):
        ...
        for pid, pa in enumerate(patch_list):
            ...
            for p_im, p_gt in zip(p_ims, p_gts):
                    ...
                    for poly in p_polygons:
                        ...
                        if p_area > 0:
                            ...
                            if p_bbox[2] > 5 and p_bbox[3] > 5:
                                ...
                                train_ob_id += 1
                # get patch info
                p_name = label_name + '-' + str(train_im_id) + '.jpg'
                ...
                io.imsave(os.path.join(output_gt_train, p_name), p_gt_3ch)
